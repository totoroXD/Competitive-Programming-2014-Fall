...
void solve(){
	for(int i=0; i<501111; i++)G[i].clear();
	for(int x=0; x<n; x++){
		for(int y=0; y<m; y++){
			for(int d=0; d<4; d++){
				int nx=x+dx[d], ny=y+dy[d], cost = edgeCost(x,y,d);
				if(0<cost){
					add_edge(idx(x,y,d), idx(nx,ny,d), cost);
					add_edge(idx(x,y,d), idx(nx,ny,4), 2*cost);
				}
			}
			for(int d=0; d<4; d++){
				int nx=x+dx[d], ny=y+dy[d], cost = edgeCost(x,y,d);
				if(0<cost){
					add_edge(idx(x,y,4), idx(nx,ny,d), 2*cost);
					add_edge(idx(x,y,4), idx(nx,ny,4), 2*cost);
				}
			}
		}
	}
